...
const RegistrationForm: React.FC = () => {
    ...
    const handleSendOtp = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            console.log('Attempting to register OTP for:', email);
            await sendOtp({ email });
            setStep(2);
        } catch (err: any) {
            console.error('Registration Send OTP Error Details:', {
                status: err.response?.status,
                data: err.response?.data,
                message: err.message,
                config: err.config
            });
            setError(err.response?.data?.message || err.message || 'Ошибка при отправке OTP. Попробуйте снова.');
        }
    };
    ...
    const handleVerifyOtp = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            console.log('Attempting to verify registration for:', email);
            ...
            setError(err.response?.data?.message || err.message || 'Неверный OTP код. Проверьте и попробуйте снова.');
        }
    };
